...
const DestinationCard = ({ destination }: DestinationCardProps) => {
  ...
  return (
    <div className="bg-white rounded-xl overflow-hidden shadow-md transition duration-300 hover:-translate-y-1 hover:shadow-lg">
      ...
      <div className="p-6">
        <h3 className="text-xl font-bold font-montserrat mb-2">{destination.name}</h3>
        <p className="text-gray-600 mb-4">{destination.shortDescription}</p>
        <div className="flex items-center mb-4">
          {Array.isArray(destination.tags) && destination.tags.slice(0, 2).map((tag, index) => (
            <span 
              key={index} 
              className={`
                ${index === 0 ? 'mr-2' : ''}
                ${tag.includes('Cultural') ? 'bg-[#E5B25D] bg-opacity-20 text-[#E5B25D]' : 
                  tag.includes('Beaches') ? 'bg-[#006B3F] bg-opacity-20 text-[#006B3F]' : 
                  tag.includes('Nature') ? 'bg-[#006B3F] bg-opacity-20 text-[#006B3F]' : 
                  tag.includes('Markets') ? 'bg-[#CE1126] bg-opacity-20 text-[#CE1126]' : 
                  tag.includes('Safari') ? 'bg-[#E5B25D] bg-opacity-20 text-[#E5B25D]' : 
